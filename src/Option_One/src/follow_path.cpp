...
// ── 参数 ─────────────────────────────────────────────
const double LOOKAHEAD   = 0.50;   // 前视距离 (m)
const double BASE_SPEED  = 0.50;   // 基础速度 (m/s)
const double MAX_SPEED   = 0.70;   // 最大速度 (m/s)
const double MIN_SPEED   = 0.15;   // 最小速度 (m/s)
const double GOAL_TOL    = 0.20;   // 终点容差 (m)
const double STICK_SCALE = 1.25;   // 摇杆→速度比例
const double LAT_KP      = 0.30;   // 横向修正增益
const int    STUCK_LIMIT = 150;    // 卡住检测
const double STUCK_EPS   = 0.02;   // 卡住阈值
// ─────────────────────────────────────────────────────
...
int main(int argc, char** argv) {
    ...
    while (running && rclcpp::ok()) {
        ...
        // ★ 发布手柄指令
        //   leftStickAnalog[1] = 东向 (正=东)
        //   leftStickAnalog[0] = 南向 (正=南, 所以代码中为 -stick_y)
        gamepad_lcmt g;
        memset(&g, 0, sizeof(g));
        g.leftStickAnalog[1] = stick_x;
        g.leftStickAnalog[0] = -stick_y;   // y轴取反: 正摇杆=南, 负摇杆=北
